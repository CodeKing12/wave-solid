...
class SpatialNavigationService {
	...
	sortSiblingsByPriority(
		...
		return sortBy(siblings, (sibling) => {
			...
			const totalDistancePoints =
				primaryAxisDistance * MAIN_COORDINATE_WEIGHT +
	...
	constructor() {
		...
		this.setFocusDebounced = debounce(
			this.setFocus,
			AUTO_RESTORE_FOCUS_DELAY,
			{
				leading: false,
				trailing: true,
			},
	...
	getEventType(keyCode: number | string) {
		return findKey(this.getKeyMap(), (codeList) =>
	...
	smartNavigate(
		...
		if (currentComponent) {
			...
			if (nextComponent) {
				...
				if (
					!parentComponent ||
					!focusBoundaryDirections.includes(direction)
				) {
	...
	getNextFocusKey(targetFocusKey: string): string {
		...
		const children = filter(
			this.focusableComponents,
			(component) =>
				component.parentFocusKey === targetFocusKey &&
				component.focusable,
		);

		if (children.length > 0) {
			const { lastFocusedChildKey, preferredChildFocusKey } =
				targetComponent;
	...
	removeFocusable({ focusKey }: FocusableComponentRemovePayload) {
		...
		if (componentToRemove) {
			...
			const hadFocusedChild =
				this.parentsHavingFocusedChild.includes(focusKey);
			this.parentsHavingFocusedChild =
				this.parentsHavingFocusedChild.filter(
					(parentWithFocusedChild) =>
						parentWithFocusedChild !== focusKey,
				);
