...
const MediaModal = function MediaModal(props: MediaModalProps) {
	...
	const [lastFocus, setLastFocus] = createSignal<string | undefined>(
		"FAVE-BTN",
	);
	...
	return (
		...
		>
			...
			<div class="relative flex flex-col justify-center gap-14 xl:h-full xl:flex-row xl:gap-20">
				...
				<div class="modal-content hide-scrollbar relative w-full py-10 text-gray-300 duration-300 ease-in-out xl:min-w-[550px] xl:overflow-y-scroll">
					{" "}
					{/* max-w-[620px] */}
					<MediaDetails
					...
					{movieDetails()?.info_labels.mediatype === "tvshow" ? (
						...
						<div
							class={`media-streams mb-6 mt-12 opacity-100 duration-300 ease-in-out ${
								streams?.length ? "" : "w-[600px]"
							} ${
								props.authToken.length
									? ""
								...
								>
									{seasons()[props?.media._id]?.length ? (
										seasons()[props.media._id].map(
											(seriesMedia, index) =>
												seriesMedia._source.info_labels
													.mediatype === "season" ? (
													<Season
														season={seriesMedia}
														onClick={() => {
