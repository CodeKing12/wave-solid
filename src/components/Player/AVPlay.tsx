...
const AVPlay = memo(function AVPlay({
	mediaUrl,
	onPlaybackComplete,
}: AVPlayProps) {
	...
	useEffect(() => {
		if (avRef.current?.childElementCount <= 1) {
			...
			tizen.systeminfo.getPropertyValue(
				...
				function successHandler(data) {
					...
					(playerLeft =
						avRef.current.offsetLeft *
						(resolutionWidth / viewPortWidth)),
						(playerTop =
							avRef.current.offsetTop *
							(resolutionHeight / viewPortHeight)),
						(playerWidth =
							avRef.current.offsetWidth *
							(resolutionWidth / viewPortWidth)),
						(playerHeight =
							avRef.current.offsetHeight *
							(resolutionHeight / viewPortHeight));
		...
		if (mediaUrl) {
			...
			(playerLeft =
				avRef.current.offsetLeft * (resolutionWidth / viewPortWidth)),
				(playerTop =
					avRef.current.offsetTop *
					(resolutionHeight / viewPortHeight)),
				(playerWidth = 1920),
	...
	return (
		<section
			id="avplay"
			class={`avplay invisible absolute top-0 opacity-0 ${
				mediaUrl ? "!visible z-[9999] !opacity-100" : ""
			}`}
			ref={avRef}
		>
