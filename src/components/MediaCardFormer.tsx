      ...
      <div class={`w-[240px] h-[330px] rounded-xl bg-black-1 backdrop-blur-2xl bg-opacity-60 cursor-pointer group relative overflow-clip duration-[400ms] ease-in-out border-4 border-transparent ${focused ? "!duration-300 border-yellow-300" : ""}`} ref={ref}>
        {
            ...
            <img width={240} height={330} class="w-full h-[330px] object-cover rounded-xl opacity-75" src={displayDetails?.art.poster} alt={displayDetails?.plot} />
            : <ImageIcon size={85} class="text-yellow-300 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 fill-transparent group-hover:-fill-yellow-300 transition-all ease-linear duration-500" variant="Broken" />
            /* eslint-enable @next/next/no-img-element */
        }
        <div class={`w-full h-full absolute bottom-0 py-5 px-3 text-gray-100 bg-black bg-opacity-80 rounded-[11px] opacity-0 group-hover:opacity-100 invisible group-hover:visible ease-in-out duration-[400ms] ${focused ? "!duration-300 !visible !opacity-100" : ""}`} onClick={() => showMediaInfo(true)}>
            <div class="flex flex-col justify-between h-full">
                <div>
                    <h5 class="text-[17px] font-medium mb-1 group-hover:text-yellow-300 duration-300 ease-linear">{ displayDetails?.title || media.info_labels?.originaltitle }</h5>
                        ...
                        <div class="flex gap-0.5">
                            {
                                Array(Math.round(rating)).fill("").map((value, index) => {
                                    return (
                                        <Star1 class="fill-yellow-300 text-yellow-300" key={index} size={18} />
                                    )
                                })
                            }
                            {
                                Array(5 - Math.round(rating)).fill("").map((value, index) => {
                                    return (
                                        <Star1 class="fill-gray-300 text-gray-300 opacity-90" key={index} size={18} />
                                    )
                                })
                            }
                        </div>
                ...
                <div class="flex items-center justify-between">
                    <button class="p-3 flex items-center gap-4 bg-[rgba(249,249,249,0.20)] backdrop-blur-[5px] rounded-2xl text-[#F9F9F9] text-lg tracking-wide font-bold border-none !outline-none hover:bg-[#F9F9F9] hover:text-black-1 group">
                        <Heart width={20} class="group-hover:-fill-black-1" />
                    </button>
