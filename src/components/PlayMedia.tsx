...
const PlayMedia = memo(function PlayMedia({ show, mediaUrl, mediaFormat, mediaType, mediaDetails, onExit }: PlayMediaProps) {
    ...
    return (
        <FocusContext.Provider value={focusKey}>
            <div class={`fixed w-full h-full top-0 bottom-0 left-0 duration-500 ease-linear opacity-0 invisible bg-black -bg-opacity-90 ${show ? "!visible !opacity-100" : ""} ${focused ? "player-is-focused" : "naa-not-focused"}`} ref={ref}>
                {/* <MediaPlayer
                    class="h-full"
                ...
                <FocusLeaf class="quitPlayer absolute top-0 right-0 z-[99999]" focusedStyles="[&>button]:!bg-black-1 [&>button]:!text-yellow-300 [&>button]:!border-yellow-300" isFocusable={Boolean(mediaUrl?.length)} onEnterPress={onExit}>
                    <button class="w-10 h-10 bg-yellow-300 border-[3px] border-transparent flex items-center justify-center hover:bg-black-1 hover:text-yellow-300 hover:border-yellow-300" onClick={onExit}>
                        <Back variant="Bold" />
                    </button>
                </FocusLeaf>
            </div>
