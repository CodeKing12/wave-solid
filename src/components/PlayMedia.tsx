...
const PlayMedia = memo(function PlayMedia({
	show,
	mediaUrl,
	mediaFormat,
	mediaType,
	...
	const { ref, focusSelf, hasFocusedChild, focusKey, focused } = useFocusable(
		{
			trackChildren: true,
	...
	return (
		<FocusContext.Provider value={focusKey}>
			<div
				class={`-bg-opacity-90 invisible fixed bottom-0 left-0 top-0 h-full w-full bg-black opacity-0 duration-500 ease-linear ${
					show ? "!visible !opacity-100" : ""
				} ${focused ? "player-is-focused" : "naa-not-focused"}`}
				ref={ref}
			>
				{/* <MediaPlayer
                    class="h-full"
...

				{
					...
					<MediaPlayer
						onCanPlay={onCanPlay}
						id={`media-player`}
						src={mediaUrl ? `${mediaUrl}.${mediaFormat}` : ""}
						class="h-full"
						title={mediaDetails?.title}
						poster={mediaDetails?.art.poster || ""}
						keyShortcuts={{
							togglePaused: "Space Enter",
							seekBackward: "ArrowLeft",
							seekForward: "ArrowRight",
							toggleFullscreen: "ArrowUp",
							toggleCaptions: "ArrowDown",
						}}
						keyTarget={show ? "document" : "player"}
						autoplay={true}
						ref={player}
					>
				...
				<FocusLeaf
					...
					isFocusable={Boolean(mediaUrl?.length)}
					onEnterPress={onExit}
				>
					<button
						class="flex h-10 w-10 items-center justify-center border-[3px] border-transparent bg-yellow-300 hover:border-yellow-300 hover:bg-black-1 hover:text-yellow-300"
						onClick={onExit}
					>
						<Back variant="Bold" />
					</button>
